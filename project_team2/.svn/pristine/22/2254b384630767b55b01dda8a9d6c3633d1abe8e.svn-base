         ...
         for (int i = 0; i < labelArrJoin.length; i++) {
            ...
            if( i<=4 ){
               textFieldJoin[i] = new JTextField(); //텍스트 필드 4개 생성　　	
						    //テキストフィールドを4つ生成
               if(i==4){
            	   textFieldJoin[i].setDocument(new JTextFieldLimit(30)); //글자수 제한
               }else{
            	   textFieldJoin[i].setDocument(new JTextFieldLimit(20)); //글자수 제한
               }
      ...
      //会員情報の変更の時
         ...
         for (int i = 0; i < labelArrMove.length; i++) {
            ...
            if(i<=5){
               ...
               textFieldMove[i].setDocument(new JTextFieldLimit(20)); //글자수 제한
               
               if(i==5){ //이메일 필드
            	   textFieldMove[i].setDocument(new JTextFieldLimit(30)); //글자수 제한
               }
